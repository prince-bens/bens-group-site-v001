...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (deliveryData.Statut_Trajet?.toLowerCase() !== 'en cours') {
            displayStatusMessage(`Le suivi en direct n'est pas disponible. Statut de la livraison : <strong>${deliveryData.statut_driver || 'A venir'}</strong>`);
            LoadingAnimation.stop();
            return;
        }
...
function updateUI(deliveryData) {
    ...
    const status = deliveryData.statut_driver || 'A venir';
...
async function updateMapWithFullPath(allPoints, userPos) {
    ...
    if (!allPoints || allPoints.length === 0) {
        ...
        // No points to display, and user marker is not requested.
        return;
    }
...
function setupRealTimeUpdates() {
    ...
    socket.on('delivery_update', (updatedData) => {
        ...
        if (updatedData.statut_driver) {
            const status = updatedData.statut_driver;
            document.querySelectorAll('.statut').forEach(el => {
                el.textContent = status;
                el.className = `statut ${status.toLowerCase().replace(/ /g, '-')}`;
            });
